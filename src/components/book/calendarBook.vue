<template>
  <div>
    ...
    <div >
      <Card :class="{'modelCard':$store.state.modal}" style="float:left;max-width:20%" >
      ...
      <Card :class="{'modelCard':$store.state.modal}" style="float:left;width:70%" >
        <full-calendar :events="fcEvents" changeMonth="changeMonth" locale="en" @dayClick="clickDay"></full-calendar>
      </Card>
    </div>
    <Card :class="{'modelCard':$store.state.modal}" style="clear:both;" >
      <table>
    ...
    getInfo() {
      ...
      tools.easyfetch(tools.Api.ListBook, this.search).then(res => {
        ...
        for (let item of this.books.list) {
          ...
          let obj = { end: end, start: begin, title: item.applicantNickname+'*'+item.deviceName };
