  ...
  <div>
    <Card class="ms-depth-16">
      ...
      <ui-button
        ...
        :loading="iswaitting"
      >{{$t('message.modify')}} {{$t('message.superPassword')}}</ui-button>
      <ui-button
        color="primary"
        icon="fingerprint"
        @click="inputSuper"
...
export default {
  ...
  methods: {
    ...
    updateClick() {
      this.item.adminSuperPassword = tools.sha3(
        this.item.adminSuperPassword.toString()
      );
      this.$Loading.start();
      ...
      tools
        .easyfetch(tools.Api.UpdateUser, this.item)
        .then(() => {
          this.$Loading.finish();
          this.$Notice.success({
            title: "Success"
          ...
          this.getUserInfo();
          this.$refs["SuperPasswordmModal"].close();
        })
        .catch();
    },
    modalClose() {
